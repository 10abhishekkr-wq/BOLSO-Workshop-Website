...
function bolso_db(): ?PDO
{
    ...
    try {
        ...
        $host = bolso_config('db_host');
        if (str_contains($exception->getMessage(), 'Access denied') && ($host === '127.0.0.1' || $host === 'localhost')) {
            try {
                $localDsn = sprintf('mysql:host=%s;port=%s;dbname=bolso;charset=utf8mb4', $host, bolso_config('db_port'));
